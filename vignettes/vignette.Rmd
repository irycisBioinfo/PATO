...
often offer do not justify the excess computational time invested. The main 
...
obtaining all those mutations (SNPs) found in regions common to all genomes. 
...
clustering_files <- dir("nr_list/fna/",pattern = "fna", full.names = T) %>% as.data.frame()
...
for(i in 1:ncol(clusters_matrix))
{
  for (j in 1:ncol(clusters_matrix))
  {
    results <- data.frame(Source = colnames(clusters_matrix)[i],
               ...
               value = adjustedRandIndex(clusters_matrix[,i],clusters_matrix[,j])) %>% bind_rows(results)
  }
}
results %>% spread(Target,value) %>% column_to_rownames("Source") %>% as.matrix() %>% pheatmap::pheatmap(display_numbers = T)

```

![](./benchClustering.png)

The results show that the methods implemented in PATO and SnapClust provide very similar results to the phylognetic groups proposed by Clermont.
